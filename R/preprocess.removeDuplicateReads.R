...
#'
#' @return invisibly; writes deduplicated fastq.gz files to output.directory
#'   and a CSV summary to logs/removeDuplicateReads_summary.csv.
#'
#' @export
...
removeDuplicateReads = function(input.reads = NULL,
  ...
  # input.reads = "read-processing/decontaminated-reads"
  # output.directory = "read-processing/deduped-reads"
  ...
  for (i in 1:length(sample.names)) {
    ...
    for (j in 1:length(sample.reads)){
      ...
      system(paste0(fastp.path, "fastp --in1 ",lane.reads[1], " --in2 ", lane.reads[2],
                    ...
                    " --html dedup_fastp.html --json dedup_fastp.json --compression 6",
                    " --report_title ", sample.reads[j]," --thread ", threads),
             ignore.stderr = quiet, ignore.stdout = quiet)

      system(paste0("cp ", "dedup_fastp.html ",
                    report.path, "/", lane.name, "_dedup_fastp.html"))
      system(paste0("rm dedup_fastp*"))
    ...
    print(paste0(sample.names[i], " Completed deduplication removal!"))

  }#end sample i loop

  write.csv(summary.data, file = paste0("logs/removeDuplicateReads_summary.csv"), row.names = FALSE)
}

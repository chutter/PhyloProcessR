  ...
  for (i in 1:length(sample.names)) {
    ...
    for (j in 1:length(sample.reads)){
      ...
      system(paste0(fastp.path, "fastp --in1 ",lane.reads[1], " --in2 ", lane.reads[2],
                    ...
                    " --html error-correct_fastp.html --json error-correct_fastp.json --compression 6",
                    " --report_title ", sample.reads[j]," --thread ", threads),
             ignore.stderr = quiet, ignore.stdout = quiet)

      system(paste0("cp ", "error-correct_fastp.html ",
                    report.path, "/", lane.name, "_error-correct_fastp.html"))
      system(paste0("rm error-correct_fastp*"))
    ...
    print(paste0(sample.names[i], " Completed read error correction!"))

  }#end sample i loop

  write.csv(summary.data, file = paste0("logs/readErrorCorrection_summary.csv"), row.names = FALSE)
}

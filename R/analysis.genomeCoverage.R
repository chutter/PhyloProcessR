...
#'
#' @description Maps paired-end reads from a single sample to a genome assembly
#'   using BWA and then summarises read depth across contigs with mosdepth.
#'   Reports per-contig mean and median depth and the proportion of bases covered
#'   at 1x, 2x, 3x, 4x, 5x, 10x, and 20x thresholds. Results are written to a
...
#'
#' @param read.directory path to the directory containing paired FASTQ files
#'   (gzipped). Files are matched by patterns \code{READ1|_R1|_1_} and
#'   \code{READ2|_R2|_2_}.
#'
#' @param output.directory path to the directory where BAM files, mosdepth
#'   output, and the coverage summary CSV will be written. Created if absent.
#'
...
#'   \code{mosdepth}. If \code{NULL} they are expected on the system PATH.
#'
...
#'   \code{<sample_name>_coverage-summary.csv} to \code{output.directory}.
#'
#' @export
